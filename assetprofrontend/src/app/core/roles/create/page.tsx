...
import type { CreateRoleDto, Permission, BreadcrumbItem, GroupedPermissions } from '@/types/core';
...
// ============================================================================
// ACTION CONFIG - Colors and Icons for each action type (matching original)
// ============================================================================
...
export default function CreateRolePage() {
  ...
  // Load permissions grouped by Module → Category
  useEffect(() => {
    const loadPermissions = async () => {
      try {
        setPermissionsLoading(true);
        setPermissionsError(null);
        console.log('[CreateRole] Loading grouped permissions...');
        ...
        console.log('[CreateRole] Loaded', allPerms.length, 'permissions across', response.moduleCount, 'modules');
      } catch (err: unknown) {
        console.error('[CreateRole] Failed to load permissions:', err);
  ...
  const isPermissionSelected = (permId: number) => formData.permissionIds.includes(permId);
  ...
  // Set category to read-only (only view permissions)
  ...
  // Set category to full access (all permissions)
  ...
  const selectAllReadOnly = () => {
    ...
    setFormData((prev) => ({
      ...prev,
      permissionIds: viewPerms.map((p) => p.id),
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await rolesApi.create({ ...formData, branchId: selectedBranchId });
      useFlashStore.getState().setFlash('Saved successfully');
      router.push('/core/roles');
    } catch (err: unknown) {
      const message = extractErrorMessage(err, 'Failed to create role');
  ...
  // Count selected permissions for a category
  const getCategorySelectedCount = (permissions: Permission[]) => {
    return permissions.filter((p) => formData.permissionIds.includes(p.id)).length;
  };
